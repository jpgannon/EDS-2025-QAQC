...
library(shinyjs)
library(DT)
library(later)  # for delaying the busy flag reset
        ...
        div(class = "sidebar-wrapper",
            ...
            wellPanel(
              ...
              fluidRow(
                ...
                column(4, actionButton("save_changes", "Save Changes", class = "btn btn-success", width = "100%"))
              ),
              br()
            ),
            br(),
...
server <- function(input, output, session) {
  rv <- reactiveValues(
    ...
    removal_range = NULL,
    nudgeMode = FALSE,
    busy = FALSE  # flag to block rapid nudge events
  )
  ...
  output$static_plot <- renderPlot({
    ...
    if (!is.null(input$comparison_columns) && length(input$comparison_columns) > 0) {
      ...
      p <- p + geom_line(data = comp_df, aes(x = .data[[date_col]], y = value, color = parameter),
                         linewidth = 1, alpha = 0.25)
    }
    p <- p + geom_line(data = edit_df_all, aes(x = .data[[date_col]], y = value, color = parameter),
                       linewidth = 1, alpha = 1) +
      geom_point(data = edit_df_all, aes(x = .data[[date_col]], y = value, color = parameter),
                 size = 1.25)
    # Highlight data points (in dark blue) that fall within the current removal range.
    if (!is.null(rv$removal_range)) {
      p <- p + geom_point(data = edit_df_all %>%
  ...
  # Debounce the brush input so it only triggers once after brushing is done.
  brush_vals <- reactive(input$plot_brush)
  debounced_brush <- debounce(brush_vals, 300)
  
  observeEvent(debounced_brush(), {
    req(debounced_brush()$xmin, debounced_brush()$xmax)
    # Reset nudge mode so new brushes always overwrite previous ones.
    rv$nudgeMode <- FALSE
    rv$removal_range <- as.POSIXct(c(debounced_brush()$xmin, debounced_brush()$xmax),
  ...
  observeEvent(input$cancel_removal, {
    ...
    showNotification("Removal canceled.", type = "warning")
  })
  ...
  output$precip_plot <- renderPlot({
    ...
    ggplot(rv$original %>% filter(.data[[date_col]] >= input$date_slider[1],
      ...
      labs(x = "Date", y = "Precipitation", title = "Precipitation (Inverted Bar Graph)") +
      theme_classic()
  })
  ...
  output$temp_plot <- renderPlot({
    ...
    ggplot(rv$original %>% filter(.data[[date_col]] >= input$date_slider[1],
      ...
      labs(x = "Date", y = "Air Temperature", title = "Air Temperature (Line Graph)") +
      theme_classic()
  })
